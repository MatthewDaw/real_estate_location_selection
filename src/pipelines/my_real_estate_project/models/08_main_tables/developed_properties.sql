-- models/08_main_tables/developed_properties.sql

with unified_developed_properties as (
    select * from {{ ref('developed_properties_summaries') }}
),

final_with_smoothed as (
    select
        row_number() over (order by udp.source, udp.source_id) as unified_id,
        udp.*
    from unified_developed_properties udp
),
...
final_with_all_smoothed as (
    select
        ...
        interior_sqft * sv.avg_effective_rent_price_per_sqft as estimated_monthly_income_sqft,
        interior_sqft * sv.avg_effective_rent_price_per_sqft * avg_leased_percentage as estimated_monthly_income_sqft_lease_percentage
